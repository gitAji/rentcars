...
function EditCarPage() {
  ...
  const handleAddBlock = async (e: React.FormEvent) => {
    ...
    if (newBlockEnd < newBlockStart) {
      setBlockedDatesError(t('admin_end_date_after_start_error'));
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(t('admin_car_updated_alert'));
  ...
  return (
    <>
      <h1 className="text-3xl font-bold mb-6 text-gray-800">{t('admin_edit_car_title')}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-lg shadow-md">
        ...
        >
          {loadingSubmit ? t('admin_updating_car') : t('admin_save_changes')}
        </button>
      </form>

      {/* Blocked Dates */}
      <div className="mt-8 bg-white p-8 rounded-lg shadow-md">
        <h2 className="text-2xl font-bold text-gray-800 mb-1">{t('admin_blocked_dates_title')}</h2>
        <p className="text-sm text-gray-500 mb-6">
          {t('admin_blocked_dates_desc')}
        </p>
        ...
        <form onSubmit={handleAddBlock} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div>
            <label htmlFor="blockStart" className="block text-sm font-medium text-gray-700 mb-1">{t('admin_field_start_date')}</label>
            <input
              type="date"
              id="blockStart"
              value={newBlockStart}
          ...
          <div>
            <label htmlFor="blockEnd" className="block text-sm font-medium text-gray-700 mb-1">{t('admin_field_end_date')}</label>
            <input
              type="date"
              id="blockEnd"
              value={newBlockEnd}
          ...
          <div>
            <label htmlFor="blockReason" className="block text-sm font-medium text-gray-700 mb-1">{t('admin_field_reason_optional')}</label>
            <input
              type="text"
              id="blockReason"
          ...
          <div className="flex items-end">
            ...
            >
              {addingBlock ? t('admin_adding_block') : t('admin_add_block')}
            </button>
          </div>
        </form>
        ...
        {blockedDates.length === 0 ? (
          <div className="flex flex-col items-center gap-2 py-8 text-gray-400">
            ...
            <p className="text-sm">{t('admin_no_blocked_dates')}</p>
          </div>
        ) : (
          <ul className="divide-y divide-gray-100">
            {blockedDates.map((block) => (
              <li key={block.id} className="flex items-center justify-between py-3">
                ...
                >
                  <FaTrash size={13} /> {t('admin_remove')}
                </button>
              </li>
            ))}
          </ul>
